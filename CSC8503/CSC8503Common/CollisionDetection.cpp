...
bool CollisionDetection::RayBoxIntersection(const Ray&r, const Vector3& boxPos, const Vector3& boxSize, RayCollision& collision) {
	...
	//RayDir loop above assumes the ray is inf both backwards and forwards, so objects interest 'behind' us
	if (bestT < 0.0f)
		return false; // no backwards rays

	Vector3 intersection = rayPos + (rayDir * bestT);
	const float epsilon = 0.0001f; //an amount of leeway in calcs
...
bool CollisionDetection::RaySphereIntersection(const Ray&r, const Transform& worldTransform, const SphereVolume& volume, RayCollision& collision) {
	...
	//Get direction between ray and sphere origins
	Vector3 dir = (spherePos - r.GetPosition());

	//Project sphere origin onto ray direction
	...
	sNorm = cos(DegreesToRadians(sNorm * 90));
